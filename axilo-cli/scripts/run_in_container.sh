...
# Remove any existing container named 'axilo'.
docker rm -f axilo || true

# Run the container with the specified directory mounted at the same path inside the container.
docker run --name axilo -d \
  -e KHULNASOFT_API_KEY \
  --cap-add=NET_ADMIN \
  --cap-add=NET_RAW \
  -v "$WORK_DIR:$WORK_DIR" \
  axilo \
...
docker exec axilo bash -c "sudo /usr/local/bin/init_firewall.sh"
