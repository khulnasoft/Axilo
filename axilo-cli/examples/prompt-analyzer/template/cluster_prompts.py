...
2.  Create embeddings via the KhulnaSoft API (``text-embedding-3-small`` by
...
def parse_cli() -> argparse.Namespace:  # noqa: D401
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Embed, cluster and analyse text prompts via the KhulnaSoft API.",
    ...
    parser.add_argument(
        ...
        help="KhulnaSoft embedding model to use.",
    )
    parser.add_argument(
        "--chat-model",
        default="gpt-4o-mini",
        help="KhulnaSoft chat model for cluster descriptions.",
    )
...
def embed_texts(texts: Sequence[str], model: str, batch_size: int = 100) -> list[list[float]]:
    """Embed *texts* with KhulnaSoft and return a list of vectors.

    Uses batching for efficiency but remains on the safe side regarding current
    KhulnaSoft rate limits (can be adjusted by changing *batch_size*).
    """

    openai = _lazy_import_openai()
    client = openai.KhulnaSoft()
...
def load_or_create_embeddings(
    ...
    * Missing embeddings are requested from the KhulnaSoft API and subsequently
