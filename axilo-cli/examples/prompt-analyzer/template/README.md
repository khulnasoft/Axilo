...
list of prompts with the KhulnaSoft Embedding API, discovers natural groupings with
...
2. Export your KhulnaSoft API key (**required**):

```bash
export KHULNASOFT_API_KEY="sk‑..."
```
...
| `--embedding-model`    | `text-embedding-3-small` | any KhulnaSoft embedding model                                                                        |
...
- **Authentication errors** – make sure `KHULNASOFT_API_KEY` is exported in the
