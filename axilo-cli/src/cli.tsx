...
// Call this early so `tail -F "$TMPDIR/oai-axilo/axilo-cli-latest.log"` works
...
const cli = meow(
  ...
  Options
    ...
    --no-project-doc           Do not automatically include the repository's 'axilo.md'
  ...
  Examples
    $ axilo "Write and run a python program that prints ASCII art"
    $ axilo -q "fix build issues"
`,
  {
    ...
    flags: {
      ...
      approvalMode: {
        ...
        aliases: ["a"],
        description:
          "Determine the approval mode for Axilo (default: suggest) Values: suggest, auto-edit, full-auto",
      },
      noProjectDoc: {
        type: "boolean",
        description: "Disable automatic inclusion of project‑level axilo.md",
      },
      projectDoc: {
        type: "string",
...
if (quietMode) {
  process.env["AXILO_QUIET_MODE"] = "1";
  if (!prompt || prompt.trim() === "") {
    // eslint-disable-next-line no-console
    console.error(
      'Quiet mode requires a prompt string, e.g.,: axilo -q "Fix bug #123 in the foobar project"',
    );
    process.exit(1);
  }
  await runQuietMode({
