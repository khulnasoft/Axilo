...
import KhulnaSoft, { APIConnectionTimeoutError } from "openai";
...
export class AgentLoop {
  ...
  // Using `InstanceType<typeof KhulnaSoft>` sidesteps typing issues with the
  // KhulnaSoft package under the TS 5+ `moduleResolution=bundler` setup.
  // KhulnaSoft client instance. We keep the concrete type to avoid sprinkling
  // `any` across the implementation while still allowing paths where the
  // KhulnaSoft SDK types may not perfectly match. The `typeof KhulnaSoft` pattern
  // captures the instance shape without resorting to `any`.
  private oai: KhulnaSoft;
  ...
  /**
   * A reference to the currently active stream returned from the KhulnaSoft
  ...
  private async handleFunctionCall(
    ...
    // The KhulnaSoft "function_call" item may have either `call_id` (responses
    ...
    // *next* request to the KhulnaSoft API – it has not been delivered yet.  If
  ...
  public async run(
    ...
    try {
      ...
      while (turnInput.length > 0) {
        ...
        // Send request to KhulnaSoft with retry on timeout
        let stream;
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
          try {
            ...
            // accommodate the test environment's minimal KhulnaSoft mocks which
            // do not define the class.  Falling back to `false` when the
            // export is absent ensures the check never throws.
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const ApiConnErrCtor = (KhulnaSoft as any).APIConnectionError as  // eslint-disable-next-line @typescript-eslint/no-explicit-any
              | (new (...args: any) => Error)
              | undefined;
            ...
            if (
              ...
              log(
                `KhulnaSoft request failed (attempt ${attempt}/${MAX_RETRIES}), retrying...`,
              );
              continue;
            }
            const isRateLimit =
            ...
            if (isRateLimit) {
              this.onItem({
                ...
                content: [
                  {
                    type: "input_text",
                    text: "⚠️  Rate limit reached while contacting KhulnaSoft. Please wait a moment and try again.",
                  },
                ],
              });
              this.onLoading(false);
                ...
                content: [
                  {
                    ...
                    text: (() => {
                      ...
                      return `⚠️  KhulnaSoft rejected the request${
      ...
      //   • the KhulnaSoft SDK attached an HTTP `status` >= 500 indicating a
      //     server‑side problem.
      ...
      const isNetworkOrServerError = (() => {
        ...
        // Direct instance check for connection errors thrown by the KhulnaSoft SDK.
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const ApiConnErrCtor = (KhulnaSoft as any).APIConnectionError as  // eslint-disable-next-line @typescript-eslint/no-explicit-any
          | (new (...args: any) => Error)
          | undefined;
        ...
        // When the KhulnaSoft SDK nests the underlying network failure inside the
      ...
      if (isNetworkOrServerError) {
        try {
          const msgText =
            "⚠️  Network error while contacting KhulnaSoft. Please check your connection and try again.";
          this.onItem({
            id: `error-${Date.now()}`,
            type: "message",
            role: "system",
...
const prefix = `You are operating as and within the Axilo CLI, a terminal-based agentic coding assistant built by KhulnaSoft. It wraps KhulnaSoft models to enable natural language interaction with a local codebase. You are expected to be precise, safe, and helpful.
You can:
...
- More details on your functionally are available at \`axilo --help\`

The Axilo CLI is open-sourced. Don't confuse yourself with the old Axilo language model built by KhulnaSoft many moons ago (this is understandably top of mind for you!). Within this context, Axilo refers to the open-source agentic coding interface.
