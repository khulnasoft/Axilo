...
import { KHULNASOFT_TIMEOUT_MS, KHULNASOFT_BASE_URL } from "../utils/config";
import {
  generateDiffSummary,
  generateEditSummary,
...
const PROMPT_HISTORY_KEY = "__axilo_singlepass_prompt_history";
...
/**
 * Small animated spinner shown while the request to KhulnaSoft is in‑flight.
 */
...
export function SinglePassApp({
  ...
  async function runSinglePassTask(userPrompt: string) {
    ...
    try {
      ...
      const openai = new KhulnaSoft({
        apiKey: config.apiKey ?? "",
        baseURL: KHULNASOFT_BASE_URL || undefined,
        timeout: KHULNASOFT_TIMEOUT_MS,
      });
  ...
  if (state === "error") {
    return (
      <Box flexDirection="column">
        <Text color="red">Error calling KhulnaSoft API.</Text>
        <ContinuePrompt
          onResult={(cont) => {
            if (!cont) {
              setQuietExit(true);
        ...
        <Box borderStyle="round" flexDirection="column" paddingX={1} width={80}>
          <Text>
            <Text bold color="magenta">
              KhulnaSoft <Text bold>Axilo</Text>
            </Text>{" "}
